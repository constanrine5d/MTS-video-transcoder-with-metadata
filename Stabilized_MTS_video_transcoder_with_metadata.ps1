...
$ffmpegTextBoxSettings.Text = "-vf vidstabtransform=input=transforms.trf:zoom=0:smoothing=10,unsharp=5:5:0.8:3:3:0.4 -copy_unknown -map_metadata 0 -map 0:0 -map 0:1 -map 1:s -c:v libx265 -crf 23 -c:a aac -b:a 256k -c:s mov_text -threads 0"
...
$ffmpegStabTextBoxSettings.Text = "-vf vidstabdetect=stepsize=6:shakiness=5:accuracy=15:result=transforms.trf -f null -"
...
Write-Output "`nThe following settings will be used for ffmpeg vidstabdetect: $ffmpegStabSettings and ffmpeg encoding: $ffmpegSettings `nand for subtitle edit: $subtitleEditSettings`n"

"The following settings will be used for ffmpeg vidstabdetect: $ffmpegStabSettings ffmpeg encoding: $ffmpegSettings `nand for subtitle edit: $subtitleEditSettings`n" | Out-File -FilePath "$selectedDirectory\MTS_video_transcoder_output.txt" -Append
...
foreach ($file in $mtsFiles) {
  ...
  # 2.1 Run FFMPEG for stabilization analysis w/ inputs: video output: transforms.trf


  # Set the input and output filenames
  #$inputFile = $file

  Write-Output "2.1 Run FFMPEG for stabilization analysis w/ inputs: video output: transforms.trf, `n Settings: $ffmpegStabSettings"
  ...
  # 2.2 Run FFMPEG w/ inputs: video and srt subtitles, output: video mp4
  ...
  Write-Output "2.2 $inputFile Run FFMPEG w/ inputs: video and srt subtitles, output: $outputFile"
